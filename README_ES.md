...
2. [Plataforma multilenguaje](#plataforma-multilenguaje)
...
9. [Vistas del juego](#vistas-del-juego)
10. [Panel de administración](#panel-de-administración)
11. [Modo por equipos](#modo-por-equipos)
12. [Generador IA](#generador-ia)
13. [Exportación y visualización de resultados](#exportación-y-visualización-de-resultados)
14. [Sistema de puntuación](#sistema-de-puntuación)
15. [Reconexión y sesiones](#reconexión-y-sesiones)
16. [Backup automático y restauración](#backup-automático-y-restauración)
17. [Estructura del proyecto](#estructura-del-proyecto)
18. [Tests y cobertura](#tests-y-cobertura)
19. [Tareas pendientes](#tareas-pendientes)
...
- **Frontend multilenguaje (ES/EN)** con diccionarios en runtime, selector de idioma con bandera y persistencia de preferencia
...
XIRO incorpora una capa i18n de frontend para español, inglés y 8 idomas más en todas las vistas.

### Comportamiento en runtime

- Orden de resolución del idioma: query en URL (`?lang=`) -> `localStorage` clave `xiro_lang` -> idioma del navegador -> fallback `es`
- El selector de la esquina inferior derecha muestra bandera + nombre del idioma (por ejemplo `🇪🇸 Español`, `🇬🇧 English`)
- El selector usa fondo semitransparente estilo glass para integrarse mejor en distintas pantallas
- También se puede cambiar por código en runtime con `window.XiroI18n.setLanguage('en', true)`

### Archivos i18n

- Runtime principal: `app/public/js/i18n/i18n-core.js`
- Traducción automática del DOM y actualizaciones dinámicas: `app/public/js/i18n/i18n-dom.js`
- Selector de idioma e integración de UI: `app/public/js/i18n/i18n-ui.js`
- Diccionarios raíz: `app/public/js/i18n/language_es.json`, `app/public/js/i18n/language_en.json`
- Diccionarios de literales por include: `app/public/js/i18n/language_es_literals.json`, `app/public/js/i18n/language_en_literals.json`

### Flujo de mantenimiento de diccionarios
...
- `extract-i18n-literals.js` escanea HTML/JS de frontend y actualiza el inventario de literales
- `build-i18n-literals.js` reconstruye los includes y conserva traducciones EN existentes
- `translate-en-literals.js` completa automáticamente literales pendientes ES -> EN (útil tras cambios grandes de contenido)

### Notas

- Para ocultar el selector en una página concreta: `<body data-i18n-switcher="off">`
- En la configuración Docker Compose por defecto (bind mount de `app` dentro del contenedor backend), cambios de textos/diccionarios de frontend suelen requerir solo hard refresh del navegador, no restart del contenedor
...
| **Validación** | Joi 18 |
| **Métricas** | Endpoint JSON de snapshot (`GET /api/metrics`) — no tiene formato Prometheus |
...
1. Al cargar `/admin.html`, la app consulta `GET /api/admin-auth/status`.
2. Si no existen usuarios (`setupRequired=true`), se muestra el formulario de registro para crear la primera cuenta.
3. Comportamiento de `POST /api/admin-register`:
  - Primer usuario registrado: se crea como `admin`, queda verificado al momento y entra directamente.
  - Usuarios adicionales: quedan en registro pendiente y deben confirmar por email.
4. La confirmación por correo usa `GET /api/admin-register/confirm?token=...`.
5. El acceso habitual usa `POST /api/admin-login` con `usuario + contraseña` (solo pueden entrar usuarios activos y verificados).
...
- Por pregunta: límite de tiempo, multimedia (texto/imagen/audio), **imagen de enunciado** (≤ 100×100 px / 25 KB), justificación, pista (numérico), estrategia de puntuación
...
### Sistema de licencias (Config → Licencia)
...
- El estado público de ejecución se expone por `GET /api/license/public-status` y lo consumen todos los clientes.
- Se muestra un badge global arriba a la izquierda en todas las vistas cuando el estado público es sin licencia.
- El estado público se comprueba al menos una vez al día y se refresca inmediatamente cuando cambia `site_settings.updated_at`.

Aplicación del modo sin licencia:

- Máximo 5 participantes por sesión (HOST + 4 jugadores).
- El límite se aplica en los flujos de unión (`join-lobby` y `join-presenter-lobby`).
- El reclaim/reconexión de un jugador previamente desconectado se permite sin consumir un slot adicional.

Rutas de licencia:

| Método | Ruta | Propósito |
|--------|------|-----------|
| `GET` | `/api/admin/license` | Leer clave persistida y estado de validación (solo admin) |
| `POST` | `/api/admin/license` | Guardar clave y revalidar (solo admin) |
| `GET` | `/api/license/public-status` | Obtener estado público actual licensed/unlicensed |
...
- [ ] **Completar traducción**. Queda por traducir los placeholder de los inputs mediante el sistema de internacionalización (i18n). "Creado por", "IA Lista", "Subir Documento", "Escribir tema", "Cada cuántos ms el servidor elimina partidas inactivas de la memoria. 3 600 000 ms = 1 hora. Valores muy bajos aumentan la carga del servidor."
...
- **Panel de monitorización integrado (Prometheus + Grafana)** — el endpoint `GET /api/metrics` expone un snapshot JSON propio (no formato Prometheus); adoptar Prometheus requeriría instrumentar `prom-client` además de añadir Grafana como servicio Docker, lo que aumenta considerablemente los requisitos de memoria y complejidad del stack para un despliegue self-hosted en NAS.
